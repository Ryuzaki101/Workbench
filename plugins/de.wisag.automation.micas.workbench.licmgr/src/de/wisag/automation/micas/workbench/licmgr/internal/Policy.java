package de.wisag.automation.micas.workbench.licmgr.internal;

import org.eclipse.core.runtime.ILog;
import org.eclipse.core.runtime.IStatus;
import org.eclipse.core.runtime.Status;

public class Policy {

    public static IStatus errorStatus(Throwable e) {
        return new Status(IStatus.ERROR, Activator.PLUGIN_ID, e.getMessage(), e);
    }

    public static void logError(Throwable e) {
        log(errorStatus(e));
    }

    private static ILog getLog() {
        return Activator.getDefault().getLog();
    }

    public static void logWarning(String message) {
        log(new Status(IStatus.WARNING, Activator.PLUGIN_ID, message, null));
    }

    public static void logError(String message) {
        log(new Status(IStatus.ERROR, Activator.PLUGIN_ID, message, null));
    }

    public static IStatus errorStatus(String message, Throwable cause) {
        return new Status(IStatus.ERROR, Activator.PLUGIN_ID, message, cause);

    }

    public static IStatus errorStatus(String message) {
        return new Status(IStatus.ERROR, Activator.PLUGIN_ID, message);

    }

    public static IStatus infoStatus(String message) {
        return new Status(IStatus.INFO, Activator.PLUGIN_ID, message);

    }

    public static void logInfo(String message) {
        log(new Status(IStatus.INFO, Activator.PLUGIN_ID, message, null));
    }

    public static void log(IStatus status) {
        getLog().log(status);

    }

}
