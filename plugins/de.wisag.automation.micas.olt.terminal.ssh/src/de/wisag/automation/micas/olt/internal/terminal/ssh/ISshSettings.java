...
public interface ISshSettings {
    ...
    /**
     * Get the password for connecting to the remote system. May be empty if
     * connecting via SSH public key authentication (with or without
     * passphrase).
     *
    ...
    /**
     * Get the timeout (in seconds) after which the SSH connection is assumed
     * dead.
     *
    ...
    /**
     * Get the keepalive interval (in seconds). After this time of inactivity,
     * the SSH connector will send a message to the remote system in order to
     * avoid timeouts on the remote. A maximum of 6 keepalive messages will be
     * sent if enabled. When set to 0, the keepalive feature is disabled.
     *
