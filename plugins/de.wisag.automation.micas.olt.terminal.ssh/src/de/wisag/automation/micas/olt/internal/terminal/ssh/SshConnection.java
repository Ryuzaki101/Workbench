...
class SshConnection extends Thread {
    ...
    /**
     * Create a Jsch session. Subclasses can override in order to replace the
     * UserInfo wrapper (for non-interactive usage, for instance), or in order
     * to change the Jsch config (for instance, in order to switch off strict
    ...
    protected Session createSession(String username, String password, String hostname, int port, UserInfo wrapperUI,
        ...
        session.setServerAliveCountMax(6); // give up after 6 tries (remote will
                                           // be dead after 30 min)
        if (password != null) {
    ...
    static void shutdown() {
        // TODO: Store all Jsch sessions in a pool and disconnect them on
        // shutdown
    }
    ...
    public void run() {
        ...
        try {
            ...
            // ui.aboutToConnect();
            if (nKeepalive > 0) {
                session.setServerAliveInterval(nKeepalive); // default is 5
                                                            // minutes
            }
    ...
    private void readDataForever(InputStream in) throws IOException {
        ...
        while ((n = in.read(bytes)) != -1) {
            /*
             * if (bytes != null && bytes.length > 0) { byte[] x =
             * ArrayUtils.subarray(bytes, 0, n);
             * System.err.println("SSH RECEIVE:"); HexDump.dump(x, 0,
             * System.out, 0); }
             */
    ...
    private class MyUserInfo implements UserInfo, UIKeyboardInteractive {
        ...
        public boolean promptYesNo(final String str) {
            ...
            Display.getDefault().syncExec(new Runnable() {
                ...
                public void run() {
                    ...
                    // retval[0] = MessageDialog.openQuestion(null,
                    // SshMessages.WARNING, str);
                    if (isSessionConnected()) {
        ...
        public void showMessage(final String message) {
            Display.getDefault().syncExec(new Runnable() {
                ...
                public void run() {
                    // [168197] Replace JFace MessagDialog by SWT MessageBox
                    // MessageDialog.openInformation(null, SshMessages.INFO,
                    // message);
                    if (isSessionConnected()) {
