...
import de.wisag.automation.micas.olt.internal.terminal.actions.TerminalAction;
import de.wisag.automation.micas.olt.internal.terminal.actions.TerminalActionConnect;
import de.wisag.automation.micas.olt.internal.terminal.actions.TerminalActionDisconnect;
import de.wisag.automation.micas.olt.internal.terminal.actions.TerminalActionNewTerminal;
import de.wisag.automation.micas.olt.internal.terminal.actions.TerminalActionRemove;
import de.wisag.automation.micas.olt.internal.terminal.actions.TerminalActionScrollLock;
import de.wisag.automation.micas.olt.internal.terminal.actions.TerminalActionSelectionDropDown;
import de.wisag.automation.micas.olt.internal.terminal.actions.TerminalActionSettings;
import de.wisag.automation.micas.olt.internal.terminal.actions.TerminalActionToggleCommandInputField;
...
import de.wisag.automation.micas.olt.internal.terminal.control.TerminalViewControlFactory;
import de.wisag.automation.micas.olt.internal.terminal.control.actions.TerminalActionClearAll;
import de.wisag.automation.micas.olt.internal.terminal.control.actions.TerminalActionCopy;
import de.wisag.automation.micas.olt.internal.terminal.control.actions.TerminalActionCut;
import de.wisag.automation.micas.olt.internal.terminal.control.actions.TerminalActionPaste;
import de.wisag.automation.micas.olt.internal.terminal.control.actions.TerminalActionSelectAll;
...
import de.wisag.automation.micas.olt.internal.terminal.provisional.api.LayeredSettingsStore;
import de.wisag.automation.micas.olt.internal.terminal.provisional.api.Logger;
import de.wisag.automation.micas.olt.internal.terminal.provisional.api.PreferenceSettingStore;
...
import de.wisag.automation.micas.olt.internal.terminal.view.ITerminalViewConnectionManager.ITerminalViewConnectionFactory;
...
public class TerminalView extends ViewPart implements ITerminalView, ITerminalViewConnectionListener {
    ...
    /**
     * This listener updates both, the view and the ITerminalViewConnection.
     */
    ...
    /**
     * Display a new Terminal view. This method is called when the user clicks
     * the New Terminal button in any Terminal view's toolbar.
     */
    @Override
    ...
    public void onTerminalNewView() {
        try {
            // The second argument to showView() is a unique String identifying
            // the
            // secondary view instance. If it ever matches a previously used
            // secondary
            // view identifier, then this call will not create a new Terminal
            // view,
            // which is undesirable. Therefore, we append the active time in
            // milliseconds to the secondary view identifier to ensure it is
            // always
            // unique. This code runs only when the user clicks the New Terminal
            // button, so there is no risk that this code will run twice in a
            // single
            // millisecond.
            IViewPart newTerminalView = getSite().getPage().showView(
                    "de.wisag.automation.micas.olt.terminal.view.TerminalView", //$NON-NLS-1$
    ...
    private ITerminalConnector showSettingsDialog(String title) {
        // When the settings dialog is opened, load the Terminal settings from
        // the
        // persistent settings.
    ...
    public void createPartControl(Composite wndParent) {
        // Bind plugin.xml key bindings to this plugin. Overrides global
        // Control-W key
        // sequence.
    ...
    /**
     * The preference setting store is used to save the settings that are shared
     * between all views.
     *
    ...
    /**
     * TODO REMOVE This code (added 2008-06-11) Legacy code to real the old
     * state. Once the state of the terminal has been saved this method is not
     * needed anymore. Remove this code with eclipse 3.5.
     */
