 ...
 * Martin Oberhuber (Wind River) - [206884] Update Terminal Ownership ID to "de.wisag.automation.micas.olt.terminal.serial"
...
public class SerialConnectWorker extends Thread {
    ...
    void addPort(String name) {
        ...
        // the port scan.
        // Unfortunately, setting gnu.io.rxtx.SerialPorts only temporarily does
        // not
        ...
        // Reinitialize the ports because we have changed the list of known
        // ports
     ...
     * "de.wisag.automation.micas.olt.internal.terminal.serial".
     *
     * @since de.wisag.automation.micas.olt.terminal.serial 2.0
     * @return ownership ID, "de.wisag.automation.micas.olt.terminal.serial"
     */
    public static final String getOwnershipId() {
        return "de.wisag.automation.micas.olt.terminal.serial"; //$NON-NLS-1$
    }
    ...
    public void run() {
        ...
        try {
            ...
            if (serialPort != null) {
                ...
                // but exceptions need to be caught to ensure that close()
                // really succeeds
                try {
                    serialPort.removeEventListener();
                    Thread.sleep(50); // allow a little time for RXTX Native to
                                      // catch up - makes stuff more stable
                } catch (Exception e) {
