 ...
 *******************************************************************************/
package de.wisag.automation.micas.olt.internal.terminal.serial;
...
public class SerialConnector extends TerminalConnectorImpl {
    ...
    public void initialize() throws Exception {
        try {
            ...
            throw new CoreException(
                    new Status(IStatus.WARNING, Activator.PLUGIN_ID, 0, SerialMessages.ERROR_LIBRARY_NOT_INSTALLED, e));
        }
    }
    ...
    public void connect(ITerminalControl control) {
        ...
        synchronized (this) {
            if (fConnectWorker != null || fDisconnectGoingOn) {
                // avoid multiple background connect/disconnect threads at the
                // same time
                return;
            }
    ...
    public void doDisconnect() {
        synchronized (this) {
            // avoid multiple background connect/disconnect threads at the same
            // time
        ...
        // Fix for SPR 112422. When output is being received from the serial
        // port, the
        // below call to removePortOwnershipListener() attempts to lock the
        // serial port
        // object, but that object is already locked by another Terminal view
        // thread
        // waiting for the SWT display thread to process a syncExec() call.
        // Since this
        // method is called on the display thread, the display thread is waiting
        // to
        // lock the serial port object and the thread holding the serial port
        // object
        // lock is waiting for the display thread to process a syncExec() call,
        // so the
        ...
        // removePortOwnershipListener(), thus preventing the display thread
        // from
            ...
            public void run() {
                try {
                    if (getSerialPortIdentifier() != null) {
                        try {
                            getSerialPortIdentifier().removePortOwnershipListener(getSerialPortHandler());
                        } catch (Exception e) {
                    ...
                    if (getSerialPort() != null) {
                        ...
                        // but exceptions need to be caught to ensure that
                        // close() really succeeds
                        try {
                            getSerialPort().removeEventListener();
                            Thread.sleep(50); // allow a little time for RXTX
                                              // Native to catch up - makes
                                              // stuff more stable
                        } catch (Exception e) {
                            Logger.logException(e);
                        }
    ...
    protected void setSerialPortIdentifier(CommPortIdentifier serialPortIdentifier) {
        // System.out.println("setSerialPortId:
        // "+Thread.currentThread().getName()+ " - "+serialPortIdentifier + " -
