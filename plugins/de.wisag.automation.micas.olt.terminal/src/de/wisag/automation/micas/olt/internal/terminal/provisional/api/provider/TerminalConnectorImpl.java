 ...
 *******************************************************************************/
package de.wisag.automation.micas.olt.internal.terminal.provisional.api.provider;
 ...
 * registered via the
 * <code>de.wisag.automation.micas.olt.terminal.terminalConnectors</code>
 * extension point.
 *
...
public abstract class TerminalConnectorImpl {
    ...
    /**
     * The TerminalControl associated with this connector. Required for
     * advertising state changes when needed.
     */
     ...
     *
     * @throws Exception
     *             when the connector fails to initialize (due to missing
    ...
    /**
     * Connect using the current state of the settings. This method is designed
     * to be overridden by actual implementations, in order to open the streams
     * required for communicating with the remote side. Extenders must call
     * <code>super.connect(control)</code> as the first thing they are doing.
     *
     * @param control
     *            Used to inform the UI about state changes and messages from
     *            the connection.
     */
     ...
     *         be sent to the remote site). For the stream in the other
     *         direction (remote to terminal see
     *         {@link ITerminalControl#getRemoteToTerminalOutputStream()}
     */
    abstract public OutputStream getTerminalToRemoteStream();

    /**
     * @return A string that represents the settings of the connection. This
     *         representation may be shown in the status line of the terminal
     *         view.
     */
     ...
     * <code>null</code> if it cannot be configured. The dialog should persist
     * its settings with the {@link #load(ISettingsStore)} and
     * {@link #save(ISettingsStore)} methods.
     *
     ...
     * {@link #connect(ITerminalControl)}. Connectors that have nothing to
     * configure do not need to implement this. Those terminals that do have
     * configuration (which they expose via {@link #makeSettingsPage()} need to
     * override this method to load settings.
     *
     * @param store
     *            a string based data store. Short keys like "foo" can be used
     ...
     *
     * @param store
     *            the store for persisting settings.
     */
