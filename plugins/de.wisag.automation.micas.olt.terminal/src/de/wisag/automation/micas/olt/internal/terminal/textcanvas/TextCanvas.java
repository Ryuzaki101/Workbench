...
/**
 * A cell oriented Canvas. Maintains a list of "cells". It can either be
 * vertically or horizontally scrolled. The CellRenderer is responsible for
 * painting the cell.
 */
public class TextCanvas extends GridCanvas {
    ...
    /**
     * Create a new CellCanvas with the given SWT style bits. (SWT.H_SCROLL and
     * SWT.V_SCROLL are automatically added).
     */
    ...
    /**
     * The user has to drag the mouse to at least one character to make a
     * selection. Once this is done, even a one char selection is OK.
     *
     * @param e
     */
    ...
    void setSelection(Point p) {
        if (fDraggingStart != null && !p.equals(fDraggingEnd)) {
            ...
            if (compare(p, fDraggingStart) < 0) {
                // bug 219589 - make sure selection start coordinates are
                // non-negative
    ...
    /**
     * Gets notified when the visible size of the terminal changes. This should
     * update the model!
     */
    ...
    /**
     * @param listener
     *            this listener gets notified, when the size of the widget
     *            changed. It should change the dimensions of the underlying
     *            terminaldata
     */
    public void addResizeHandler(ResizeListener listener) {
        ...
        // Bug 281328: [terminal] The very first few characters might be missing
        // in
        // the terminal control if opened and connected programmatically
        //
        // In case the terminal had not been visible yet or is too small (less
        // than one
        // line visible), the terminal should have a minimum size to avoid
        // RuntimeExceptions.
        Rectangle bonds = getClientArea();
        if (bonds.height < getCellHeight() || bonds.width < getCellWidth()) {
            ...
            // Widget realized: compute actual size and force telling the
            // listener
            onResize(true);
        }
    }
    ...
    /**
     * @return true if the cursor is enabled (blinking). By default the cursor
     *         is not enabled.
     */
