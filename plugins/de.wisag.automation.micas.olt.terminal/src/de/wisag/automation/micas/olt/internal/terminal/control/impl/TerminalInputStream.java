...
/**
 * The main purpose of this class is to start a runnable in the display thread
 * when data is available and to pretend no data is available after a given
 * amount of time the runnable is running.
 */
public class TerminalInputStream extends InputStream {
    ...
    /**
     * A shared timer for all terminals. This times is used to limit the time
     * used in the display thread....
     */
    ...
    /**
     * The maximum amount of data read and written in one shot. The timer cannot
     * interrupt reading this amount of data. {@link #available()} and
     * {@link #read(byte[], int, int)} This is used as optimization, because
     * reading single characters can be very inefficient, because each call is
     * synchronized.
     */
    ...
    /**
     * Used as flag to indicate that the current runnable has used enough time
     * in the display thread. This variable is set by a timer thread after the
     * Runnable starts to run in the Display thread after {@link #fUITimeout}.
     */
    // synchronized with fQueue!
    private boolean fEnoughDisplayTime;

    /**
     * A byte bounded buffer used to synchronize the input and the output
     * stream.
     * <p>
     ...
     * Lea (ISBN 0-201-31009-0). Second edition published by Addison-Wesley,
     * November 1999. The code is Copyright(c) Douglas Lea 1996, 1999 and
     * released to the public domain and may be used for any purposes
     * whatsoever.
     * <p>
    ...
    class BoundedByteBuffer {
         ...
         *             when the Thread is interrupted while waiting for the
         *             buffer to become available because it was full
         */
         ...
         * @throws InterruptedException
         *             when the Thread is interrupted while waiting for the
         *             buffer to be filled with a readable byte
         */
    ...
    /**
     * @param bufferSize
     *            the size of the buffer of the output stream
     * @param uiTimeout
     *            the maximum time the notifyChange runnable runs. It will be
     *            rescheduled after uiTimeout if input data is still available.
     * @param notifyChange
     *            a Runnable that is posted to the Display thread via
     *            {@link Display#asyncExec}. The runnable is posted several
     *            times!
     */
    public TerminalInputStream(int bufferSize, int uiTimeout, Runnable notifyChange) {
        // if(true) {notifyChange=new Runnable() {public void run() {byte
        // buff[]=new byte[1024];while((available())>0)try {read(buff);} catch
        // (IOException e) {break;}}};}
    ...
    /**
     * Posts the runnable {@link #fNotifyChange} to the display Thread, unless
     * the runnable is already scheduled. It will make {@link #ready} return
     * false after {@link #fUITimeout} milli seconds.
     */
    void bytesAreAvailable() {
        ...
        synchronized (fQueue) {
            if (fRunnable == null) {
                ...
                // TODO: make sure we don't create a display if the display is
                // disposed...
    ...
    /**
     * Starts a timer that sets {@link #fEnoughDisplayTime} to true after
     * milliSec.
     *
     * @param milliSec
     *            The time after which fEnoughDisplayTime is set to true.
     */
    ...
    /**
     * @return the next available byte. Check with {@link #available} if
