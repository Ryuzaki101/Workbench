...
public interface ITerminalControl {
     ...
     * remote. By default, the local Platform Default Encoding is used. Also
     * note that the encoding must not be applied in case the terminal stream is
     * processed by some data transfer protocol which requires binary data.
     ...
     * processed. The concrete encoding to use can either be specified manually
     * by a user, by means of a dialog, or a connector can try to obtain it
     * automatically from the remote side e.g. by evaluating an environment
     * variable such as LANG on UNIX systems.
     *
     ...
     *         stream appear in the terminal or are interpreted by the emulator
     *         as control sequences. The stream in the opposite direction,
     *         terminal to remote is in
     ...
     *
     * @param msg
     *            TODO: Michael Scharf: Should be replaced by a better error
    ...
    /**
     * void setQuiet(boolean quiet); void
     * sendCommandWithCallback(IDeviceReadCallback dataProcessor);
     */
