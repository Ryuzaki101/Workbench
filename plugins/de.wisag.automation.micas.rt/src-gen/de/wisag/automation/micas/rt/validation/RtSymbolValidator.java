/**
 * <copyright>
 * </copyright>
 *
 * $Id$
 */
package de.wisag.automation.micas.rt.validation;

import org.eclipse.emf.common.util.EList;

import de.wisag.automation.micas.rt.RtModule;
import de.wisag.automation.micas.rt.RtSymbolAccess;

/**
 * A sample validator interface for
 * {@link de.wisag.automation.micas.rt.RtSymbol}. This doesn't really do
 * anything, and it's not a real EMF artifact. It was generated by the
 * org.eclipse.emf.examples.generator.validator plug-in to illustrate how EMF's
 * code generator can be extended. This can be disabled with -vmargs
 * -Dorg.eclipse.emf.examples.generator.validator=false.
 */
public interface RtSymbolValidator {
    boolean validate();

    boolean validateAddress(int value);

    boolean validateName(String value);

    boolean validateModule(RtModule value);

    boolean validateAccessors(EList<RtSymbolAccess> value);

    boolean validateIndex(int value);
}
