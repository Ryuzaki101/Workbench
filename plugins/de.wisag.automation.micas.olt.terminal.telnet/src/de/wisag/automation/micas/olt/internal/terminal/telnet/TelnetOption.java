...
/**
 * This class represents a single TELNET protocol option at one endpoint of a
 * TELNET connection. This class encapsulates the endpoint associated with the
 * option (local or remote), the current state of the option (enabled or
 * disabled), the desired state of the option, the current state of the
 * negotiation, an OutputStream that allows communication with the remote
 * endpoint, and the number of negotiations that have started within this
 * connection.
 * <p>
 * In addition to encapsulating the above state, this class performs option
 * negotiation to attempt to achieve the desired option state. For some options,
 * this class also performs option sub-negotiation.
 * <p>
 * IMPORTANT: Understanding this code requires understanding the TELNET protocol
 * and TELNET option processing.
 * <p>
 *
...
class TelnetOption implements TelnetCodes {
    /**
     * This array of Strings maps an integer TELNET option code value to the
     * symbolic name of the option. Array elements of the form "?" represent
     * unassigned option values.
     */
     ...
     * <p>
     * This constant and the others having similar names represent the states of
     * a finite state automaton (FSA) that tracks the negotiation state of this
     * option. The initial state is NEGOTIATION_NOT_STARTED. The state machine
     * is as follows (with transitions labeled with letters in parentheses):
     * <p>
     *
     * <pre>
     ...
     * </pre>
     * <p>
     * Once the FSA leaves state NEGOTIATION_NOT_STARTED, it never returns to
     * that state. Transition A happens when the local endpoint sends an option
     * command before receiving a command for the same option from the remote
     * endpoint.
     * <p>
     * Transition B happens when the local endpoint receives a reply to an
     * option command sent earlier by the local endpoint. Receipt of that reply
     * terminates the negotiation.
     * <p>
     * Transition D happens after negotiation is done and "something changes"
     * (see the RFCs for the definition of "something changes"). Either endpoint
     * can re-negotiate an option after a previous negotiation, but only if some
     * external influence (such as the user or the OS) causes it to do so.
     * Re-negotiation must start more than {@link #NEGOTIATION_IGNORE_DURATION}
     * milliseconds after the FSA enters state NEGOTIATION_DONE or it will be
     * ignored. This is how this client prevents negotiation loops.
     * <p>
     * Transition C happens when the local endpoint receives an option command
     * from the remote endpoint before sending a command for the same option. In
     * that case, the local endpoint replies immediately with an option command
     * and the negotiation terminates.
     * <p>
     * Some TELNET servers (e.g., the Solaris server), after sending WILL and
     * receiving DONT, will reply with a superfluous WONT. Any such superfluous
     * option command received from the remote endpoint while the option's FSA
     * is in state {@link #NEGOTIATION_DONE} will be ignored by the local
     * endpoint.
     */
    ...
    /**
     * The number of milliseconds following the end of negotiation of this
     * option before which the remote endpoint can re-negotiate the option. Any
     * option command received from the remote endpoint before this time passes
     * is ignored. This is used to prevent option negotiation loops.
     *
     ...
     * terminated (i.e., entered state {@link #NEGOTIATION_DONE}). This is used
     * to determine whether an option command received from the remote endpoint
     * after negotiation has terminated for this option is to be ignored or
     * interpreted as the start of a new negotiation.
     *
    ...
    /**
     * Holds the total number of negotiations that have completed for this
     * option.
     */
    ...
    /**
     * Holds the OutputStream object that allows data to be sent to the remote
     * endpoint of the TELNET connection.
     */
    protected OutputStream outputStream;

    /**
     * True if this option is for the local endpoint, false for the remote
     * endpoint.
     */
    ...
    /**
     * This field is true if the client desires the option to be enabled, false
     * if the client desires the option to be disabled. This field does not
     * represent the remote's endpoints desire (as expressed via WILL and WONT
     * commands) -- it represnet the local endpoint's desire.
     * <p>
     *
    ...
    /**
     * @return Returns a String containing the name of the TELNET option
     *         specified in parameter <i>option</i>.
     */
    ...
    /**
     * Returns true if the local endpoint desires this option to be enabled,
     * false if not. It is not an error for the value returned by this method to
     * differ from the value returned by isEnabled(). The value returned by this
     * method can change over time, reflecting the local endpoint's changing
     * desire regarding the option.
     * <p>
     * NOTE: Even if this option represents a remote endpoint option, the return
     * value of this method represents the local endpint's desire regarding the
     * remote option.
     * <p>
     *
     * @return Returns true if the local endpoint desires this option to be
    ...
    /**
     * Sets our desired value for this option. Note that the option can be
     * desired when <i>enabled</i> is false, and the option can be undesired
     * when <i>enabled</i> is true, though the latter state should not persist,
     * since either endpoint can disable any option at any time.
     * <p>
     *
     * @param newValue
     *            True if we desire this option to be enabled, false if we
     *            desire this option to be disabled.
     */
    ...
    /**
     * Call this method to request that negotiation begin for this option. This
     * method does nothing if negotiation for this option has already started or
     * is already complete. If negotiation has not yet started for this option
     * and the local endpoint desires this option to be enabled, then we send a
     * WILL or DO command to the remote endpoint.
     */
    ...
    public void handleWill() {
        ...
        if (negotiationState == NEGOTIATION_IN_PROGRESS) {
            if (desired) {
                // We sent DO and server replied with WILL. Enable the option,
                // and end
                // this negotiation.
                ...
                // This should never happen! We sent DONT and the server replied
                // with
                // WILL. Bad server. No soup for you. Disable the option, and
                // end
                // this negotiation.
            ...
            if (desired) {
                // Server sent WILL, so we reply with DO. Enable the option, and
                // end
                // this negotiation.
                ...
                // Server sent WILL, so we reply with DONT. Disable the option,
                // and
    ...
    /**
     * Handles a WONT command sent by the remote endpoint for this option. The
     * value of <i>desired</i> doesn't matter in this method, because the remote
     * endpoint is forcing the option to be disabled.
     */
    ...
    public void handleDo() {
        ...
        if (negotiationState == NEGOTIATION_IN_PROGRESS) {
            if (desired) {
                // We sent WILL and server replied with DO. Enable the option,
                // and end
                // this negotiation.
                ...
                // We sent WONT and server replied with DO. This should never
                // happen!
            ...
            if (desired) {
                // Server sent DO, so we reply with WILL. Enable the option, and
                // end
                // this negotiation.
                ...
                // Server sent DO, so we reply with WONT. Disable the option,
                // and end
                // this negotiation.
    ...
    /**
     * Handles a DONT command sent by the remote endpoint for this option. The
     * value of <i>desired</i> doesn't matter in this method, because the remote
     * endpoint is forcing the option to be disabled.
     */
    public void handleDont() {
        ...
        if (negotiationState == NEGOTIATION_IN_PROGRESS) {
            ...
            // Server sent DONT, so we reply with WONT. Disable the option, and
            // end
            // this negotiation.
    ...
    /**
     * This method handles a subnegotiation command received from the remote
     * endpoint. Currently, the only subnegotiation we handle is when the remote
     * endpoint commands us to send our terminal type (which is "ansi").
     *
     * @param subnegotiationData
     *            An array of bytes containing a TELNET subnegotiation command
     *            received from the remote endpoint.
     * @param count
     *            The number of bytes in array subnegotiationData to examine.
     */
    public void handleSubnegotiation(byte[] subnegotiationData, int count) {
        ...
        case TELNET_OPTION_TERMINAL_TYPE:
            ...
            // Tell the remote endpoint our terminal type is "ansi" using this
            // sequence
            // of TELNET protocol bytes:
            //
            // IAC SB TERMINAL-TYPE IS a n s i IAC SE

            byte[] terminalTypeData = { TELNET_IAC, TELNET_SB, TELNET_OPTION_TERMINAL_TYPE, TELNET_IS, (byte) 'a',
     ...
     *
     * @param subnegotiationData
     *            An array of Objects holding data to be used when generating
     *            the outbound subnegotiation command.
     */
    public void sendSubnegotiation(Object[] subnegotiationData) {
        ...
        case TELNET_OPTION_NAWS:
            ...
            NAWSData[3] = (byte) ((width >>> 8) & 0xff); // High order byte of
                                                         // width.
            NAWSData[4] = (byte) (width & 0xff); // Low order byte of width.
            NAWSData[5] = (byte) ((height >>> 8) & 0xff); // High order byte of
                                                          // height.
            ...
            // This final local variable is a hack to get around the fact that
            // inner
            // classes cannot reference a non-final local variable in a
            // lexically
            // enclosing scope.

            final byte[] NAWSDataFinal = NAWSData;

            // Send the NAWS data in a new thread. The current thread is the
            // display
            // thread, and calls to write() can block, but blocking the display
            // thread
    ...
    /**
     * This method returns true if there has not yet been any negotiation of
     * this option.
     *
     * @return Returns true if there has not yet been any negotiation of this
     *         option.
     */
    ...
    /**
     * This method terminates the current negotiation and records the time at
     * which the negotiation terminated.
     */
     ...
     * interpret as the start of a new negotiation. If the superfluous command
     * is not ignored, an option negotiation loop can result (which is bad). For
     * details about the superfluous commands sent by some servers, see the
     * documentation for {@link #NEGOTIATION_NOT_STARTED}.
     * <p>
     * The current implementation of this method returns true if the new
     * negotiation starts within NEGOTIATION_IGNORE_DURATION seconds of the end
     * of the previous negotiation of this option.
     * <p>
     *
     * @return Returns true if the new negotiation should be ignored, false if
     *         not.
     */
    ...
    /**
     * This method sends a WILL/WONT/DO/DONT command to the remote endpoint for
     * this option.
     */
