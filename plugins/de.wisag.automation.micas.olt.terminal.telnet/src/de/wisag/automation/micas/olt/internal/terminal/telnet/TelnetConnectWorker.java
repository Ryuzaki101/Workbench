...
class TelnetConnectWorker extends Thread {
    ...
    public void run() {
        ...
        while (remaining >= 0) {
            // Pause before we re-try if the remaining tries are less than the
            // initial value
            if (remaining < 10) {
                try {
                    Thread.sleep(500);
            ...
            try {
                ...
                // data
                // inline with the rest of the non-urgent data. Without this
                // call, TCP
                // urgent data is silently dropped by Java. This is required for
                // TELNET support, because when the TELNET server sends "IAC
                // DM", the
                // IAC byte is TCP urgent data. If urgent data is silently
                // dropped, we
                // only see the DM, which looks like an ISO Latin-1 '�'
                // character.
                ...
                // No re-try in case of UnknownHostException, there is no
                // indication that
                ...
                // Time out occurred. No re-try in this case either. Time out
                // can
                // be increased by the user. Multiplying the timeout with the
                // remaining
                ...
                // In case of a ConnectException, do a re-try. The server could
                // have been
