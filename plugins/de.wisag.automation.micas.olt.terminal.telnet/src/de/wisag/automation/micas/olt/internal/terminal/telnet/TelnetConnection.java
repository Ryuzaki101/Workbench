...
public class TelnetConnection extends Thread implements TelnetCodes {
     ...
     * {@link #STATE_SUBNEGOTIATION_STARTED} and
     * {@link #STATE_RECEIVING_SUBNEGOTIATION}.
     */
        ...
        try {
            while (socket.isConnected()) {
                ...
                if (nRawBytes == -1) {
                    ...
                    // send any TELNET protocol data until we are sure the
                    // remote
    ...
    protected void initializeOptions() {
        ...
        // disabled (since either endpoint can force any option to be disabled
        // by simply
     ...
     * <i>rawBytes</i>. This function preserves its state between calls, because
     * a multi-byte TELNET command might be split between two (or more) calls to
     * this function. The state is preserved in field <i>telnetState</i>. This
     * function implements an FSA that recognizes TELNET option codes. TELNET
     * option sub-negotiation is delegated to instances of TelnetOption.
     *
    ...
    protected int processTelnetProtocol(int count) {
        ...
        for (int byteIndex = 0; byteIndex < count; ++byteIndex) {
            ...
            // that nothing happens. This happens when array rawBytes[] contains
            // no
            ...
            case STATE_IAC_RECEIVED:
                ...
                // skip all not yet processed non-TELNET-protocol data preceding
                // the
                // DM byte. However, Java 1.4.x has no way to inform clients of
                // class Socket that urgent data is available, so we simply
                // ignore the
                ...
                case TELNET_GA: // Go Ahead command. Meaningless on a
                                // full-duplex link.
                case TELNET_IP: // Interupt Process command. Server should never
                                // send this.
                case TELNET_AO: // Abort Output command. Server should never
                                // send this.
                case TELNET_AYT: // Are You There command. Server should never
                                 // send this.
                case TELNET_EC: // Erase Character command. Server should never
                                // send this.
                case TELNET_EL: // Erase Line command. Server should never send
                                // this.
                    telnetState = STATE_INITIAL;
                    break;
            ...
            case STATE_RECEIVING_SUBNEGOTIATION:
                if (inputByte == TELNET_IAC) {
                    ...
                    if (nextSubnegotiationByteIndex > 0
                        ...
                        // subnegotiation was IAC, so this is a double IAC.
                        // Leave the previous IAC
                        // in the receivedSubnegotiation[] array and drop the
                        // current
                        // one (thus translating a double IAC into a single
                        // IAC).
                    ...
                    // because we need to know when the subnegotiation ends, and
                    // that is
                ...
                // part of the subnegotiation data, so don't do anything unless
                // the SE
                ...
                if (inputByte == TELNET_SE && receivedSubnegotiation[nextSubnegotiationByteIndex - 1] == TELNET_IAC) {
                    ...
                    // process it. We always use the option object stored in
                    // array
                    // localOptions[] to process the received subnegotiation.
                    // This is an arbitrary decision, but it is sufficient for
                    // handling
                    // options TERMINAL-TYPE and NAWS, which are the only
                    // options that
                    // we subnegotiate (presently). If, in the
                    // future,subnegotiations
                    ...
                    // while receiving the subnegotiation, so we must not
                    // process the
                ...
                if (nextSubnegotiationByteIndex >= receivedSubnegotiation.length) {
                    ...
                    // subnegotiation for requires that many bytes in a
                    // subnegotiation command.
                    // In the interest of robustness, we handle this case by
                    // ignoring all
                    // remaining subnegotiation bytes until we receive the IAC
                    // SE
            ...
            default:
                ...
                // bug in this FSA. For robustness, we return to the initial
                // state.
