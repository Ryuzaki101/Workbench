...
typedef	union	{
	WORD	CW;		// overlay for bits
	struct	{
	BYTE	maxAdr;		// maximale PROFIBUS Adresse
...
typedef union	{
	...
	unsigned B3	:1;
	unsigned BKop	:1;
	unsigned GASio	:1;     // ga2000 mit ser. schnittstelle
	unsigned FKopp	:1;     // wenn keine ankopplung aktiv
	unsigned B7	:1;
	}Bit;
	}SWTYP, *PSWTYP;
...
typedef union	{
	BYTE	B;
	struct	{
	unsigned Brate	:4;     // Bitrate
	unsigned Absch	:1;     // Abschlusswiderstand
	unsigned Port	:1;
	unsigned Res	:1;
	unsigned aktiv	:1;
	}Bit;
...
typedef	struct	{
	...
	BYTE	mon;		// Monat				F2
	BYTE	year;		// Jahr					F3
	BYTE	Prufer[10];	// Name des Pr¸fers			 F4
...
#define GWMBU 0xB6	// fuer M-BUS bzw. Meter-Bus
#define GWCAN 0xBC	// fuer CAN-BUS
...
#define	MOD_canmd	29
#define	MOD_eecmd	30
#define	MOD_impsio	31
#define	MD_MAXINX	31

#define	KIP_Id		('K'+('P'<<8))
#define	ModMax		31

#define PROG_ID_LEN	8	
#define NEURON_ID_LEN	6


// Klassen fuer FG-Arten
#define	DpkMax		9
#define	Dpk1F		1
#define	Dpk2F		2
#define	Dpk3F		3
...
#define	FGA_GA		0x0000
#define	FGA_PP		0x0001
#define	FGA_DEU		0x0002

// Maskentype fuer FG-Arten
#define	MSTyp1F		1
#define	MSTyp2F		2
#define	MSTyp3F		3
#define	MSTypSA		4
...
// Struktur fuer FG-Artenbeschreibung

typedef	struct	IpartS	{
	LPSTR	ipaex;
	LPSTR	ipadeu;
	WORD	ipain;
	WORD	TInd;
...
typedef	struct	FgartS	{
	...
	BYTE	Ipanz;
	PIPART 	*Ipa;
	BYTE	KKenn;          // alte Kennung
	BYTE	KK_Out;		// Koppel-Kennung Output
	BYTE	KK_Inp;		// Koppel-Kennung Input
}FGART, *PFGART;

typedef	struct	{
	char	*Ipa;
	short	rSp;	// Bezugsspalte relativ bezogen auf Blockanfang
	short	isBSp;
	short	BSp;	// Bezugsspalte absolut bezogen auf Blockanfang
	}IPSPLT,*PIPSPLT;
...
//	E I B   -   D e f i n f i t i o n e n
//	Tabelle der KO-Typen
typedef	struct	{
	LPSTR	KOTyp;
	BYTE	KOVal;
	LPSTR	Dpkls;
	}KOTYP;
typedef	KOTYP	*PKOTYP;


//	CAN Datenstruktur
typedef	struct	{
	WORD	SID;		// Modul Stamm-ID
...
typedef	union {
	struct	{
	unsigned Port	:2;
	unsigned AbWid	:1; 	// Abschlusswiderstand
	unsigned Res	:5;
		}INF;
	BYTE	IB;
	}PORTINF;
...
//	Header
typedef	struct	{
	BYTE	HdrLen;		// Headerl‰nge
	WORD	TabLen;		// Tabellenl‰nge inkl. Header
	WORD	Chks;		// Checksumme ¸ber Tabellenl‰nge
	BYTE	LVers;		// Listenversion
	BYTE	Baud;		// Bitrate CAN-Bus
	PORTINF	PI;		// Portnummer....
	BYTE	Line;		// Modul-Info....
	BYTE	USNr;
	BYTE	ModNr;
	DWORD	Time;		// Zeitstempel der Generierung im Unix-Format
	BYTE	WD_Hg;		// Windocu-Version Hauptgruppe
	BYTE	WD_Ug;		// Untergruppe
	BYTE	WD_Rv;		// Revision
	WORD	SDOAnz;		// Anzahl Service Dataobjekte
	BYTE	Resv[10];
	WORD	Cnt;		// Anzahl Module
	MODTYP	MTypes[1];	// Benutze Module sortiert nach DE,DA,AE,AA Endekennung SID = 0;
}CANHDR,*PCANHD;
...
#ifndef	_MICAS_
ZLKls	WINAPI GetTypKls(long, WORD, WORD);
ZLKls	WINAPI GetTypKls2(long, WORD, WORD, WORD);
#endif
...
DWORD	WINAPI CNInx2Baud (WORD, WORD);
DWORD	WINAPI GABaud2Inx (DWORD, WORD);
DWORD	WINAPI CNBaud2Inx (DWORD, WORD);
...
WORD	WINAPI IsCBd_possible (LPVOID EEDat);
...
LPSTR	WINAPI GetNextModul (LPLONG, LPWORD);
LPSTR	WINAPI GetNAliModul (LPSTR, LPLONG);

PIPSPLT	WINAPI GetIpaInfo (LPSTR);
PFGART	WINAPI GetEIBDpkl (PFGART, WORD);
WORD	WINAPI GetAscDpk (WORD, LPSTR);
PIPART	WINAPI GetPIpaS (VOID);
PFGART	WINAPI GetPFgaS (VOID);
PFGART	WINAPI GetPFgart (WORD, LPSTR);
PFGART	WINAPI GetPFga2 (LPSTR);
WORD	WINAPI GetMTypFga (WORD, LPSTR);
int	WINAPI GetObjPZ (WORD);
WORD	WINAPI GetObjSpOff (WORD Dpk, WORD Sp);
WORD	WINAPI GetDpklFga (WORD, LPSTR);
WORD	WINAPI GetDpKlasse (LPSTR);
WORD	WINAPI GetEIFga (LPSTR);
WORD	WINAPI GetKOPFga (LPSTR);
WORD	WINAPI GetKoppKenn (LPSTR, WORD);
WORD	WINAPI GetFGAKenn (LPSTR Fga, WORD Out);
WORD	WINAPI GetKPDpk (LPSTR, LPSTR);
LPSTR	WINAPI GetIPIpArt (LPSTR, LPSTR);
VOID	WINAPI GetIPIpAdr (LPSTR, WORD, LPSTR, LPSTR);
LPSTR	WINAPI GetIPArtInx (LPSTR, WORD);
short	WINAPI GetBfIpAdr (LPSTR, WORD, WORD, LPSTR);
WORD	WINAPI GetBezSp (LONG, LONG, WORD);
WORD	WINAPI GetIpaInd (LPSTR);
LPSTR	WINAPI GetAscIpa (WORD);
int	WINAPI FgartBin2Asc (LPSTR, WORD, WORD);
int	WINAPI FgartAsc2Bin (LPSTR, WORD);
int	WINAPI FgartPP2GA (WORD);
int	WINAPI FgartGA2PP (WORD);
VOID	WINAPI	GetIPArten (LPSTR, LPSTR);
PKOTYP	WINAPI	GetKOTyp (LPSTR, WORD);
PKOTYP	WINAPI	NextKOTyp (PKOTYP, WORD);
VOID	WINAPI	GetDpkDirect (LPSTR, LPSTR);
LPSTR	WINAPI KopFga2Dpk (WORD Kn, LPWORD Dpk);
